...
logger.info(f"✅ BOT_TOKEN loaded: {BOT_TOKEN[:20]}...")
...
def webhook_token():
    """Handle webhook updates from Telegram using token"""
    logger.debug(f"POST request to /{BOT_TOKEN} endpoint")
    return handle_webhook()

@app.route('/webhook', methods=['POST'])
def webhook_default():
    """Handle webhook updates from Telegram using /webhook endpoint"""
    logger.debug("POST request to /webhook endpoint")
    return handle_webhook()
...
def handle_webhook():
    ...
    try:
        ...
        logger.debug(f"Webhook data received: {json_data}")
        ...
        if update.message:
            logger.info(f"Message from user {update.message.chat.id}: {update.message.text}")
        if update.callback_query:
            logger.info(f"Callback from user {update.callback_query.message.chat.id}: {update.callback_query.data}")
...
def setup_webhook():
    ...
    try:
        ...
        logger.info(f"Removing old webhook...")
        bot.remove_webhook()
        time.sleep(1)
